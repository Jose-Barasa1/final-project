...
def place_order():
    try:
        ...
        for item in data['cart_items']:
            ...
            new_order = Order(customer_id=current_user['id'], product_id=item['product_id'], status='processing')
...
def view_orders():
    try:
        ...
        return jsonify([{"id": o.id, "status": o.status, "product_id": o.product_id, "product_name": o.product.name, "delivery_status": o.delivery.delivery_status if o.delivery else 'N/A'} for o in orders]), 200
    except Exception as e:
        return jsonify({"message": f"An error occurred: {str(e)}"}), 500
...
def add_to_cart():
    try:
        ...
        if 'product_id' not in data or 'quantity' not in data:
            return jsonify({"message": "Product ID and quantity are required"}), 400
        existing_cart_item = Cart.query.filter_by(user_id=current_user['id'], product_id=data['product_id']).first()
        if existing_cart_item:
            existing_cart_item.quantity += data['quantity']
            db.session.commit()
            return jsonify({"message": "Cart updated"}), 200
        new_cart_item = Cart(user_id=current_user['id'], product_id=data['product_id'], quantity=data['quantity'])
...
def get_cart():
    try:
        ...
        return jsonify([{'product_id': item.product.id, 'product_name': item.product.name, 'price': item.product.price, 'quantity': item.quantity} for item in cart_items]), 200
    except Exception as e:
        return jsonify({"message": f"An error occurred: {str(e)}"}), 500
